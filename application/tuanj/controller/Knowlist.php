...
class Knowlist extends BasicAdmin {

    private $dataform = 'know_list';
    public $table ='know_cate';
    ...
    protected function _data_filter(&$data) {
        foreach ($data as $key => $val) {
            $data[$key]['cates'] = Db::name($this->table)->where('id', '=', $val['kid'])->value('title');
        }
    }
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/knowlist/index')];
