...
//    private $cate = 'news_cate';
    ...
    public function index() {
        $this->title = '关于我们管理';
        list($get, $db) = [$this->request->get(), Db::name($this->dataform)->where('status',2)];
...
//            $db->whereBetween('create_at', [$start_time, $end_time]);
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/gywm/index')];
